...
class IndexUpdater:
    ...
    def update_index_html(self, k_league_matches):
        ...
        # หาตำแหน่งที่จะแทรก K League 2 (หลังจาก FIFA Club World Cup หรือก่อน footer)
        if "FIFA Club World Cup" in html_content:
            # แทรกหลัง FIFA Club World Cup section
            insert_position = html_content.find('</div>\n            </div>\n        </div>', 
                                               html_content.find("FIFA Club World Cup"))
            if insert_position != -1:
                insert_position = html_content.find('</div>', insert_position) + 6
                html_content = html_content[:insert_position] + "\n" + k_league_section + html_content[insert_position:]
        else:
            # แทรกก่อน footer
        ...
        current_leagues = re.search(r'<div class="stat-number">(\d+)</div>\s*<div class="stat-label">Leagues Covered</div>', html_content)
        if current_leagues:
            current_count = int(current_leagues.group(1))
