#!/usr/bin/env python3
"""
🚀 Ultra Advanced ML Analysis for China Super League - July 18, 2025
วิเคราะห์การแข่งขัน China Super League ด้วย Ultra Advanced ML รวมถึงการวิเคราะห์เตะมุม
"""
...
class UltraAdvancedMLAnalyzer:
    """Ultra Advanced ML Analyzer for China Super League"""
    ...
    def __init__(self, input_file, output_csv):
        ...
        self.h2h_weight = 0.30
        self.stats_weight = 0.25
        self.home_advantage = 0.15
        self.recent_form_weight = 0.05
    ...
    def analyze_h2h(self, h2h_data, home_team_id):
    ...
    def analyze_recent_form(self, team_data):
        """Analyze recent form based on last 5 matches with recency bias"""
        matches = team_data.get('last_5_matches', [])
        if not matches:
            return 0.5
        
        # Apply recency bias - more recent matches have higher weight
        weights = [0.1, 0.15, 0.2, 0.25, 0.3]  # Most recent match has highest weight
        score = 0
        total_weight = 0
    ...
    def predict_match_result(self, match):
        ...
        home_recent = self.analyze_recent_form(home_team)
        away_recent = self.analyze_recent_form(away_team)
        recent_advantage = home_recent / (home_recent + away_recent) if (home_recent + away_recent) > 0 else 0.5
        ...
        home_win_prob = (
            ...
            self.home_advantage +
            recent_advantage * self.recent_form_weight
        )
    ...
    def analyze_all_matches(self):
        ...
        print("\n🔍 Analyzing matches with Ultra Advanced ML...")
    ...
    def save_updated_json(self):
        ...
        output_json = self.input_file.replace('.json', '_with_predictions.json')
    ...
    def print_summary(self):
        ...
        for pred in self.predictions:
            ...
            if pred['match_winner'] == 'HOME' and (1/pred['odds_home']) < (pred['home_win_prob']/100):
                value = round((pred['home_win_prob']/100) / (1/pred['odds_home']) - 1, 2) * 100
                if value >= 5:  # At least 5% edge
                    value_bets.append({
                        'match': f"{pred['home_team']} vs {pred['away_team']}",
                        'bet': f"Home Win @ {pred['odds_home']}",
                        'edge': f"{value:.1f}%",
                        'confidence': f"{pred['confidence']:.1f}%"
                    })
            
            elif pred['match_winner'] == 'DRAW' and (1/pred['odds_draw']) < (pred['draw_prob']/100):
                value = round((pred['draw_prob']/100) / (1/pred['odds_draw']) - 1, 2) * 100
                if value >= 5:
                    value_bets.append({
                        'match': f"{pred['home_team']} vs {pred['away_team']}",
                        'bet': f"Draw @ {pred['odds_draw']}",
                        'edge': f"{value:.1f}%",
                        'confidence': f"{pred['confidence']:.1f}%"
                    })
            
            elif pred['match_winner'] == 'AWAY' and (1/pred['odds_away']) < (pred['away_win_prob']/100):
                value = round((pred['away_win_prob']/100) / (1/pred['odds_away']) - 1, 2) * 100
                if value >= 5:
                    value_bets.append({
                        'match': f"{pred['home_team']} vs {pred['away_team']}",
                        'bet': f"Away Win @ {pred['odds_away']}",
                        'edge': f"{value:.1f}%",
                        'confidence': f"{pred['confidence']:.1f}%"
                    })
            
            # Check for value in corners market
            corners_pred = pred['corners'].split()
            corners_side = corners_pred[0]
            corners_line = float(corners_pred[1])
            ...
            if corners_side == 'OVER' and pred['corners_confidence']/100 > (1/pred['odds_corners_over']):
                ...
                if value >= 5:
                    value_bets.append({
                        'match': f"{pred['home_team']} vs {pred['away_team']}",
                        'bet': f"Corners Over {corners_line} @ {pred['odds_corners_over']}",
                        'edge': f"{value:.1f}%",
                ...
                if value >= 5:
                    value_bets.append({
                        'match': f"{pred['home_team']} vs {pred['away_team']}",
                        'bet': f"Corners Under {corners_line} @ {pred['odds_corners_under']}",
                        'edge': f"{value:.1f}%",
                        'confidence': f"{pred['corners_confidence']:.1f}%"
                    })
    ...
    def run(self):
        ...
        print("🚀 Starting Ultra Advanced ML Analysis for China Super League")
        print("=" * 100)
...
def main():
    ...
    output_csv = "/Users/80090/Desktop/Project/untitle/api_data/china_super_league/china_super_league_predictions_20250718_updated.csv"
    
    analyzer = UltraAdvancedMLAnalyzer(input_file, output_csv)
    analyzer.run()
