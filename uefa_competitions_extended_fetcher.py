#!/usr/bin/env python3
"""
🚀 UEFA Competitions Extended Fetcher - July 17-18, 2025
ดึงข้อมูลการแข่งขัน UEFA Europa League และ UEFA Europa Conference League วันที่ 17-18 กรกฎาคม 2025
"""
...
class UEFACompetitionsExtendedFetcher:
    ...
    def fetch_competition_fixtures_by_date(self, league_id: int, date: str) -> Dict:
        """ดึงข้อมูลการแข่งขันของลีกที่กำหนดตามวันที่"""
        url = f"{self.base_url}/fixtures"
        params = {'league': league_id, 'date': date}
        
        try:
            print(f"📥 กำลังดึงข้อมูลการแข่งขัน League ID: {league_id}, Date: {date}...")
    ...
    def fetch_all_competitions_extended(self) -> Dict:
        """ดึงข้อมูลทั้งหมดจากทุกการแข่งขันในวันที่ 17-18 กรกฎาคม 2025"""
        all_data = {
            'europa_league': [],
            'conference_league': [],
            'summary': {},
        ...
        # กำหนดวันที่ต้องการดึงข้อมูล (UTC)
        ...
        uel_fixtures = []
        for date in dates:
            uel_data = self.fetch_competition_fixtures_by_date(3, date)
            if uel_data:
                fixtures = self.process_fixtures(uel_data, self.competitions[3])
                uel_fixtures.extend(fixtures)
                print(f"✅ ดึงข้อมูล UEFA Europa League วันที่ {date} สำเร็จ: {len(fixtures)} รายการ")
        ...
        uecl_fixtures = []
        for date in dates:
            uecl_data = self.fetch_competition_fixtures_by_date(848, date)
            if uecl_data:
                fixtures = self.process_fixtures(uecl_data, self.competitions[848])
                uecl_fixtures.extend(fixtures)
                print(f"✅ ดึงข้อมูล UEFA Europa Conference League วันที่ {date} สำเร็จ: {len(fixtures)} รายการ")
...
def main():
    """ฟังก์ชันหลัก"""
    print("🚀 UEFA Competitions Extended Fetcher - July 17-18, 2025")
    print("=" * 60)
    ...
    # สร้าง fetcher
    fetcher = UEFACompetitionsExtendedFetcher(api_key)
    
    # ดึงข้อมูลทั้งหมด
    all_data = fetcher.fetch_all_competitions_extended()
    ...
    # บันทึกข้อมูลลงไฟล์
    output_file = "uefa_europa_competitions_extended_data.json"
