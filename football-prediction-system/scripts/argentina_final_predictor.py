#!/usr/bin/env python3
"""
Argentina Final Predictor - Ready for Tonight's Matches
ระบบทำนายสำหรับ Argentina Primera Division พร้อมทดสอบจริง
"""
...
def create_final_argentina_data():
    ...
    for i in range(20):
        ...
        corners = max(6, min(18, int(total_attack/10) + random.randint(-3, 5)))
...
class ArgentinaFinalPredictor:
    ...
    def predict_match(self, home_team, away_team):
        ...
        # Over/Under
        over_under_prob = self.models['over_under_rf'].predict_proba(features)[0][1]
        ...
        # Corners
        corners_prob = self.models['corners_rf'].predict_proba(features)[0][1]
...
def main():
    ...
    # แสดงสถิติทีม
    print(f"\n📈 Team Statistics:")
    for team, stats in list(predictor.team_stats.items())[:5]:
    ...
    for match in tonight_matches:
        prediction = predictor.predict_match(match['home'], match['away'])
        predictions_data['matches'].append({
    ...
    print("   Check back after the matches to compare with actual results")
    print("   River Plate vs Boca Juniors at 01:30")
