...
# -*- coding: utf-8 -*-
"""
🚀 Update Index with Additional European Leagues Analysis (Real Data, Fixed)
อัพเดทหน้า index.html ด้วยการวิเคราะห์ลีกยุโรปเพิ่มเติมด้วยข้อมูลจริงและเวลาไทย
"""
...
def load_more_european_html():
    """Load additional European leagues HTML report"""
    try:
        with open('more_european_leagues_report.html', 'r', encoding='utf-8') as f:
            soup = BeautifulSoup(f, 'html.parser')
            return soup
    except:
        print("Error: Could not load additional European leagues HTML report")
        return None
...
def extract_league_sections(more_european_soup):
    """Extract league sections from additional European leagues HTML report"""
    if not more_european_soup:
        return []
    
    # Find all league sections (cards)
    league_cards = more_european_soup.select('div.card.mb-4')
...
def update_index_html(index_soup, league_sections):
    """Update index.html with additional European leagues analysis"""
    ...
    # Remove any existing sections for these leagues
    for card in container.select('div.card.mb-4'):
        header = card.select_one('div.card-header h2')
        if header:
            header_text = header.text.strip()
            for league_name in ["Norway - Eliteserien", "Sweden - Allsvenskan", "Poland - Ekstraklasa", 
                               "Iceland - Úrvalsdeild", "Sweden - Superettan", "Denmark - 1. Division", 
                               "Finland - Ykkönen"]:
...
def main():
    # Load additional European leagues HTML report
    more_european_soup = load_more_european_html()
    
    if not more_european_soup:
        print("No additional European leagues HTML report found. Please run generate_more_european_report_real_fixed.py first.")
        return
    
    # Extract league sections from additional European leagues HTML report
    league_sections = extract_league_sections(more_european_soup)
    
    if not league_sections:
        print("No league sections found in additional European leagues HTML report.")
        return
    ...
    if success:
        print("Successfully updated index.html with additional European leagues analysis.")
    else:
